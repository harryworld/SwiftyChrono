...
public class ZHHansDateParser: Parser {
    ...
    override public func extract(text: String, ref: Date, match: NSTextCheckingResult, opt: [OptionType: Int]) -> ParsedResult? {
        ...
        guard let month = NSRegularExpression.isMatch(forPattern: "\\d+", in: monthString) ? Int(monthString) : ZHHANSStringToNumber(text: monthString) else {
            return nil
        }
        ...
        if match.isNotEmpty(atRangeIndex: dayGroup) {
            ...
            guard let day = NSRegularExpression.isMatch(forPattern: "\\d+", in: dayString) ? Int(dayString) : ZHHANSStringToNumber(text: dayString) else {
                return nil
            }
        ...
        if match.isNotEmpty(atRangeIndex: yearGroup) {
            ...
            guard let year = NSRegularExpression.isMatch(forPattern: "\\d+", in: yearString) ? Int(yearString) : ZHHANSStringToYear(text: yearString) else {
                return nil
            }
