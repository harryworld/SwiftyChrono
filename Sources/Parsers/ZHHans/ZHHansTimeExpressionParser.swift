...
private let FIRST_REG_PATTERN = "(?:由|從|自)?" +
    ...
    ")?" +
    "(?:[\\s,，]*)" +
    "(?:(\\d+|\(ZHHANS_NUMBER_PATTERN)+)(?:\\s*)(?:点|时|:|：)" +
    "(?:\\s*)" +
    "(\\d+|半|正|整|\(ZHHANS_NUMBER_PATTERN)+)?(?:\\s*)(?:分|:|：)?" +
    "(?:\\s*)" +
    "(\\d+|\(ZHHANS_NUMBER_PATTERN)+)?(?:\\s*)(?:秒)?)" +
    "(?:\\s*(A\\.M\\.|P\\.M\\.|AM?|PM?))?";
...
public class ZHHansTimeExpressionParser: Parser {
    ...
    override public func extract(text: String, ref: Date, match: NSTextCheckingResult, opt: [OptionType: Int]) -> ParsedResult? {
        ...
        if match.isNotEmpty(atRangeIndex: secondGroup) {
            ...
            guard let second = NSRegularExpression.isMatch(forPattern: "\\d+", in: secondString) ? Int(secondString) : ZHHANSStringToNumber(text: secondString) else {
                return nil
            }
